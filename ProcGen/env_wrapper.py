...
class ProcgenCoinRunEnvWrapper(gym.Env):
  ...
  def step(self, action): #executes pre-chosen step
   obs, reward, done, info = self.env.step(action)
   state = {
       "obs": obs,
   }
   if "progress" in info:
       state["progress"] = info["progress"] #helpful for tpe (possibly)


   try:
       custom_reward = self.reward_fn(state, action, info) #scoring each step w/in a single reward function
   except Exception as e:
       print("Reward function error:", e)
       custom_reward = 0.0


   return obs, np.array([custom_reward], dtype=np.float32), done, info #idk if we want it to see obs
